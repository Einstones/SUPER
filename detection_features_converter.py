...
def main():
    ...
    split_indices_path = os.path.join(
        config.ids_path, args.split + '36_imgid2idx.pkl')

    # load all image ids
    img_ids = []
    for split in split_set:
        split_ids_path = os.path.join(config.ids_path, split + '_ids.pkl')
        if os.path.exists(split_ids_path):
            img_ids += _pickle.load(open(split_ids_path, 'rb'))
        else:
            ...
            _pickle.dump(img_ids_dump, open(split_ids_path, 'wb'))
            img_ids += _pickle.load(open(split_ids_path, 'rb'))
    ...
    with open(bottom_up_path, 'r') as tsv_in_file:
        ...
        for item in tqdm(reader, total=num_images):
            ...
            if image_id in img_ids:
                ...
                bboxes = np.frombuffer(buf,dtype=np.float32).reshape((item['num_boxes'], -1))
                ...
                split_img_features[counter, :, :] = np.frombuffer(buf, dtype=np.float32).reshape((item['num_boxes'], -1))
    ...
    print("done!")
    _pickle.dump(indices, open(split_indices_path, 'wb'))
    h_split.close()
