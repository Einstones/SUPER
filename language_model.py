...
class WordEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        emb = self.dropout(emb)  # 600
        return emb

class ClassEmbedding(nn.Module):
    """
       Convert label to class vector
    """
    def __init__(self, nclass, emb_dim, dropout):
        super(ClassEmbedding, self).__init__()
        self.emb = nn.Embedding(nclass, emb_dim)  # (1600, 300)
        self.dropout = nn.Dropout(dropout)

    def init_embedding(self, np_file):
        weight_init = torch.from_numpy(np.load(np_file))  # (1600, 300)
...
class AttrEmbedding(nn.Module):
    """
       Convert label to class vector
    """
    def __init__(self, nattr, emb_dim, dropout):
        super(AttrEmbedding, self).__init__()
        self.emb = nn.Embedding(nattr, emb_dim)  # (400, 300)
        self.dropout = nn.Dropout(dropout)

    def init_embedding(self, np_file):
        weight_init = torch.from_numpy(np.load(np_file))  # (400, 300)
        self.emb.weight.data = weight_init
